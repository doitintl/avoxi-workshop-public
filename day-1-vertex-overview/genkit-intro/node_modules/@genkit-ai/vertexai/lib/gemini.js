...
const VertexRetrievalSchema = import_zod.z.object({
  datastore: import_zod.z.object({
    projectId: import_zod.z.string().optional(),
    location: import_zod.z.string().optional(),
    dataStoreId: import_zod.z.string()
  }),
  disableAttribution: import_zod.z.boolean().optional()
});
const GoogleSearchRetrievalSchema = import_zod.z.object({
  disableAttribution: import_zod.z.boolean().optional()
});
...
function geminiModel(name, vertexClientFactory, options) {
  var _a, _b;
  ...
  return (0, import_model.defineModel)(
    ...
    (request, streamingCallback) => __async(this, null, function* () {
      ...
      if ((_j = request.config) == null ? void 0 : _j.googleSearchRetrieval) {
        (_k = chatRequest.tools) == null ? void 0 : _k.push({
          googleSearchRetrieval: request.config.googleSearchRetrieval
        });
      }
      if ((_l = request.config) == null ? void 0 : _l.vertexRetrieval) {
        ...
        (_m = chatRequest.tools) == null ? void 0 : _m.push({
          retrieval: {
            vertexAiSearch: {
              datastore
            },
      ...
      if (streamingCallback) {
        ...
        try {
          for (var iter = __forAwait(result.stream), more, temp, error; more = !(temp = yield iter.next()).done; more = false) {
            ...
            (_n = item.candidates) == null ? void 0 : _n.forEach((candidate) => {
        ...
        if (!((_o = response.candidates) == null ? void 0 : _o.length)) {
          throw new Error("No valid candidates returned.");
        }
        return {
          candidates: ((_p = response.candidates) == null ? void 0 : _p.map(fromGeminiCandidate)) || [],
          custom: response
        };
      } else {
        let result;
        ...
        if (!((_q = result == null ? void 0 : result.response.candidates) == null ? void 0 : _q.length)) {
          throw new Error("No valid candidates returned.");
        }
        const responseCandidates = ((_r = result.response.candidates) == null ? void 0 : _r.map(fromGeminiCandidate)) || [];
        return {
          candidates: responseCandidates,
          custom: result.response,
          usage: __spreadProps(__spreadValues({}, (0, import_model.getBasicUsageStats)(request.messages, responseCandidates)), {
            inputTokens: (_s = result.response.usageMetadata) == null ? void 0 : _s.promptTokenCount,
            outputTokens: (_t = result.response.usageMetadata) == null ? void 0 : _t.candidatesTokenCount,
            totalTokens: (_u = result.response.usageMetadata) == null ? void 0 : _u.totalTokenCount
          })
        };
      }
    })
