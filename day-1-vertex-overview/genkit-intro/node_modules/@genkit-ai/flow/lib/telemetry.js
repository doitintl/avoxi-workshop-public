...
const pathCounter = new import_metrics.MetricCounter(_N("path/requests"), {
  description: "Tracks unique flow paths per flow.",
  valueType: import_api.ValueType.INT
});
const pathLatencies = new import_metrics.MetricHistogram(_N("path/latency"), {
...
function recordError(err) {
  var _a, _b, _c, _d, _e;
  const paths = ((_b = (_a = import_tracing.traceMetadataAls) == null ? void 0 : _a.getStore()) == null ? void 0 : _b.paths) || /* @__PURE__ */ new Set();
  const failedPath = ((_c = Array.from(paths).find((p) => p.status === "failure")) == null ? void 0 : _c.path) || ((_e = (_d = import_tracing.spanMetadataAls) == null ? void 0 : _d.getStore()) == null ? void 0 : _e.path) || "";
  const displayPath = (0, import_tracing.toDisplayPath)(failedPath);
  import_logging.logger.logStructuredError(`Error[${displayPath}, ${err.name}]`, {
    path: displayPath,
...
function logRequest(flowName, req) {
  var _a, _b;
  const qualifiedPath = ((_b = (_a = import_tracing.spanMetadataAls) == null ? void 0 : _a.getStore()) == null ? void 0 : _b.path) || "";
...
function writePathMetrics(flowName, latencyMs, err) {
  var _a, _b, _c;
  const paths = ((_a = import_tracing.traceMetadataAls.getStore()) == null ? void 0 : _a.paths) || /* @__PURE__ */ new Set();
  const flowPaths = Array.from(paths).filter(
    (meta) => meta.path.includes(flowName)
  );
  if (flowPaths) {
    import_logging.logger.logStructured(`Paths[${flowName}]`, {
      flowName,
    ...
    if (err && !flowPaths.some((p) => p.status === "failure")) {
      writePathMetric(flowName, {
        status: "failure",
        path: ((_c = (_b = import_tracing.spanMetadataAls) == null ? void 0 : _b.getStore()) == null ? void 0 : _c.path) || "",
        error: err,
        latency: latencyMs
      });
    }
